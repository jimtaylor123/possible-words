...
- **Analysis**: Read the GitHub issue and codebase in the worktree. Write analysis to `.agent/todo.md` and `.agent/plan.md`. Ask clarifying questions if needed.
- **Plan approval**: Present the final implementation plan. Do NOT proceed until the user explicitly approves.
- **Implementation**: Implement the approved plan in small commits. Keep all work inside the worktree only. Update `.agent/todo.md` (progress) and `.agent/notes.md` (decisions).
- **Fix loop**: Fix ONLY issues listed in `.agent/review.md`. Do not introduce new features.
